typedef struct WinMsgName {
    int code;
    char* name;
} WinMsgName;

static WinMsgName winMsgNames[] = {
{0, "WM_NULL" },
{1, "WM_CREATE" },
{2, "WM_DESTROY" },
{3, "WM_MOVE" },
...
char*
findMsgName(int msg_code) {
    char* name = 0;
    for (int msg_index = 0; msg_index < sizeof(winMsgNames) / sizeof(winMsgNames[0]); msg_index++) {
        WinMsgName msg = winMsgNames[msg_index];
        if (msg.code == msg_code) {
            name = msg.name;
            break;
        }
    }
